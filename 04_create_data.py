...
active_users = df_selected[df_selected["active"] == True]
print(active_users)
...
print(df_with_dup.duplicated(keep=False))  #keep=False: represent all duplicates, keep="frist": Kept the first one among the duplicates.
...
df_selected["signup_converted"] = pd.to_datetime(df_selected["signup_date"], errors="coerce", dayfirst=True)
 #errors="coerce": Non-convertible values will be set to "NaT", dayfirst=True: interpret dates in European format
...
"""_
    ...
    df_selected["signup_converted"] = pd.to_datetime(df_selected["signup_date_cleaned"], errors="coerce", dayfirst=True)
...
def fill_salary(x):
    return 40000.0 if pd.isna(x) else float(x)  #if x == NaN, then fill with 40000.0
...
df_piped = (
    df_selected
      .pipe(normalize_strings)  # 문자열 정규화(날짜 구분자 통일)
